...
allFiles = glob.glob(filename+"m*.cvs")

arrayMid = np.zeros(20)
arrayStandard = np.zeros(20)
arrayDistance = np.array(
    [100, 130, 160, 190, 220, 250, 280, 310, 340, 370, 400, 430, 460, 490, 520, 550, 580, 610, 640, 670])

arrayLogMid = np.zeros(20)
arrayLogStandard = np.zeros(20)
arrayLogValues = [[], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []]
...
for files in allFiles:
    ...
    arrayValues[index] = voltage
    arrayStandard[index] = np.std(voltage)
    arrayMid[index] = voltage.mean()
    print("Datei: %d, Mittelwert: %f, Standardabweichung: %f" % (index + 1, arrayMid[index], arrayStandard[index]))
    index = index + 1
...
#
# Average
axes[0, 0].plot(arrayMid, arrayDistance)
...
#
# Standardabweichung
axes[0, 1].plot(arrayDistance, arrayStandard)
...
arrayDistancePaper = np.array(
    [100, 130, 160, 190, 220, 250, 280, 310, 340, 370, 400, 430, 460, 490, 520, 550, 580, 610, 640, 670, 700])
arrayVoltagePaper = np.array(
    [1.363, 1.212, 1.078, 0.973, 0.897, 0.8215, 0.7653, 0.6992, 0.6567, 0.6374, 0.5986, 0.5604, 0.5415, 0.5227, 0.5228,
     0.5037, 0.4848, 0.4847, 0.4846, 0.4846, 0.4657])
#
# PLOT
#
axes[1, 0].plot(arrayVoltagePaper, arrayDistancePaper, 'o', markersize=3, color='red')
axes[1, 0].plot(arrayVoltagePaper, arrayDistancePaper, markersize=1, color='black', linestyle='-')
axes[1, 0].set_ylabel("Distance Log [mm]")
axes[1, 0].set_xlabel("Voltage Log [V]")
...
axes[1, 1].plot(arrayVoltagePaperLog, arrayDistancePaperLog, 'o', markersize=3, color='red')
axes[1, 1].plot(arrayVoltagePaperLog, arrayDistancePaperLog, markersize=1, color='black', linestyle='-')
axes[1, 1].set_ylabel("Distance Log [mm]")
axes[1, 1].set_xlabel("Voltage Log [V]")
axes[1, 1].set_title("Logarithmiert -> Kennlinie")
...
dinaA4LangDatei = glob.glob(filename +"dina4l.csv")
dinaA4BreitDatei = glob.glob(filename + "dina4b.csv")
