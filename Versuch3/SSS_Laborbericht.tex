...
\newcommand{\strTopic}{Versuch 3 Fourieranalyse und Akustik}
\newcommand{\strAbstract}{Im Versuch geht es um die Fourieranalyse eines akustischen Signals.
Sinn ist es, ein praktisches Verständnis für die Fourierreihe und die Fourieranalyse zu erhalten, zudem soll der geübt werden wie man akustische Signale messen kann.
Es wird spezifisch der Ton einer Klarinette aufgenommen. Das Signal des Tons wird in seinen Zeitbereich dargestellt ein repräsentativer Abschnitt des Signals wird gesucht und  dann fouriertransformiert, damit anschließend sein Frequenzbereich dargestellt werden kann. Für die Fouriertransformation werden die Funktionen aus der Numpy Bibliothek verwendet.

Aufgrund der Corona Pandemie wird der zweite Versuch zur Messung des Frequenzgangs am Lautsprecher ausgelassen.
}
...
\subsubsection*{Code für die Aufnahme (audioSample.py)}


\lstinputlisting[language=Python]{src/audioSample.py}

\subsubsection*{Code für die Auswertung (aufgabe3.py)}

\lstinputlisting[language=Python]{src/aufgabe3.py}

\section{Messergebnisse}
\label{chap:APPENDIX_MEASUREMENT_SOURCE}

\include{media/data.cv}
